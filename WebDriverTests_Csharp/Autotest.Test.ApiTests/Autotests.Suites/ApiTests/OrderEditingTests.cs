...
{
    public class OrderEditingTests : GetConstantBasePage
    {
...
//           Порверка статуса заявки
        ...
        public void OrderErrorEditingTest()
        {
            ...
            responseFail = (ApiResponse.ResponseFail)apiRequest.POST("api/v1/" + keyShopPublic + "/order_update/" +
            ...
            Assert.IsFalse(responseFail.Success, "Ожидался ответ false на отправленный запрос POST по API");
            Assert.AreEqual(responseFail.Response.ErrorText, "Email должно быть корректным адресом электронной почты");
            ...
            Assert.IsFalse(responseFail.Success, "Ожидался ответ false на отправленный запрос POST по API");
            Assert.AreEqual(responseFail.Response.ErrorText, "Ошибка просчета цены, или маршрут недоступен");
            ...
            Assert.IsFalse(responseFail.Success, "Ожидался ответ false на отправленный запрос POST по API");
            Assert.AreEqual(responseFail.Response.ErrorText, @"Длина поля &laquo;Комментарий к заказу&raquo; должна быть не более 300 символов");
            ...
            Assert.IsFalse(responseFail.Success, "Ожидался ответ false на отправленный запрос POST по API");
            Assert.AreEqual(responseFail.Response.ErrorText, @"Длина поля &laquo;Дополнительный телефон получателя&raquo; должна быть не более 30 символов");
        }
    }
}