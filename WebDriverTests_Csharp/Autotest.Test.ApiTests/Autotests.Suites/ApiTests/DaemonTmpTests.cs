...
{
    public class DaemonTmpTests : GetConstantBasePage
    {
        [Test, Description("Получить перечень городов по началу названия")]
        public void AutocompleteTest()
        {
//            Вводим название москва
            var responseCities = (ApiResponse.ResponseDeamonСities)apiRequest.GET("tmpdaemon/",
...
//            вводим начало с Санкт-Петербург
            responseCities = (ApiResponse.ResponseDeamonСities)apiRequest.GET("tmpdaemon/",
                   new NameValueCollection
                    {
                        {"_action", "autocomplete"},
                        {"q", "Санкт-Петербург"}
...
//            ничего не вовводим
            var responseFail = (ApiResponse.ResponseDeamonСities)apiRequest.GET("tmpdaemon/",
                   new NameValueCollection
                    {
                        {"_action", "autocomplete"},
                        {"q", ""}
        ...
        {
//            вводим id Снегири
            var responseCity = (ApiResponse.ResponseDeamonСity)apiRequest.GET("tmpdaemon/",
                   new NameValueCollection
                    {
                        {"_action", "city"},
                        {"_id", "712"}
...
//            вводим не сущестующий Id
            responseCity = (ApiResponse.ResponseDeamonСity)apiRequest.GET("tmpdaemon/",
                   new NameValueCollection
                    {
                        {"_action", "city"},
                        {"_id", "qwe"}
...
//            запрашиваем список точек по Санкт-Петербургу
            responseDeliveryPoints = (ApiResponse.ResponseDeamonPoints)apiRequest.GET("tmpdaemon/",
                   new NameValueCollection
                    {
                        {"_action", "delivery_points"},
                        {"cities", "151185"},
...
//            проверяем что вторая точка есть
        ...
        private ApiResponse.OptionsPoints FindRowByName(string name, ApiResponse.ResponseDeamonPoints responseDeliveryPoints)
        {
            for (var i = 0; i < responseDeliveryPoints.Points.Count(); i++)
            {
